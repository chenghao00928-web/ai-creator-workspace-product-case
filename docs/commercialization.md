...
- 对 AI 产品作品集而言，如何体现成本意识、商业化判断和产品取舍？
...
| 标准任务 | 需求解析、材料润色、会议纪要 | 2,000 | 1,200 | 0.0044 | 0.0111 |
| 重度任务 | 长材料总结、运营方案、深度任务分析 | 5,000 | 3,000 | 0.0111 | 0.0277 |
| 多版本任务 | 一次生成 3 个版本脚本/材料表达 | 2,500 | 3,600 | 0.0098 | 0.0244 |
...
| 资料处理用户 | 资料分析阶段集中使用 | 300 次标准 + 100 次多版本 + 30 次重度 | 约 6.60 |
...
| 权益 | 设计 |
|---|---|
| 适用用户 | 资料处理用户、项目实践准备用户 |
| 月度额度 | 300 次标准任务 + 100 次多版本任务 + 30 次重度任务 |
| 高级模板 | 需求解析链路、材料项目经历改写、行动清单生成 |
| 项目管理 | 最多 20 个分析项目 |
| 导出 | Markdown / Word 文案复制 |
| 付费理由 | 帮助学生围绕不同公司和任务快速生成项目材料 |
...
| 能力 | 免费版 | 学生资料分析版 | 创作者/办公专业版 | 小团队版 |
|---|---|---|---|---|
| 基础模板 | 支持 | 支持 | 支持 | 支持 |
| 高级模板 | 不支持 | 资料分析模板 | 创作/办公模板 | 团队模板 |
...
| 转化点 | 触发场景 | 推荐策略 |
|---|---|---|
| 多版本额度用完 | 用户频繁生成多个标题/脚本/材料版本 | 推荐专业版或学生资料分析版 |
| 项目数量达到上限 | 用户保存多个任务、账号或运营项目 | 推荐升级项目空间 |
| 使用高级模板 | 用户点击 需求解析链路、账号风格模板 | 展示模板价值和示例输出 |
...
AI 信息分析工作台 Demo 面向作品集展示，不应开放成无限调用的公开服务。Demo 阶段建议采用以下控制策略：

| 策略 | 说明 |
|---|---|
| 限制单次输入长度 | 前后端限制需求文本 和个人背景长度，避免一次请求消耗过多输入 Token |
| 限制输出长度 | 后端设置 `max_tokens`，控制单次输出成本 |
| 不保存用户隐私数据 | 前端仅用 localStorage 保存本地历史，不上传材料、手机号、邮箱等敏感信息 |
...
对资料处理用户，付费价值来自需求解析、材料改写和方案复盘链路；对内容创作者和运营用户，付费价值来自多版本生成、账号风格和项目复用；对团队用户，付费价值来自模板共享、项目协作和历史资产沉淀。

在 MVP 阶段，建议采用免费版降低试用门槛，用 19 元学生资料分析版和 39 元专业版验证个人用户付费意愿，再将团队版作为 P2 阶段商业化扩展方向。
