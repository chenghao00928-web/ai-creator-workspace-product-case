...
本项目是 AI 产品项目实践作品集项目，仅有产品方案、PRD 和原型还不足以体现 AI 产品落地能力。因此新增「AI 信息分析工作台 Demo」，通过真实大模型 API 完成任务 需求解析、能力差距分析、材料改写建议和行动清单生成。
...
```mermaid
flowchart LR
    A["前端表单"] --> B["POST /api/analyze-text"]
    B --> C["Express 后端校验参数"]
...
| 返回内容为空 | 模型响应异常或输入过短 | 重试或补充 需求文本 和个人背景 |
